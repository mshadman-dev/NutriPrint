...
def _build_prompt(data: dict) -> str:
    ...
    # Pull ICMR RDA targets for this age group (fall back to 9-12 if not found)
    rda = ICMR_RDA.get(data['age_group'], ICMR_RDA["9-12"])
    rda_calories  = rda["calories"]
    rda_protein   = rda["protein_g"]
    rda_calcium   = rda["calcium_mg"]
    rda_iron      = rda["iron_mg"]
...
ICMR RDA NUTRITION TARGETS for age group {data['age_group']} (MUST be met):
- Daily calories  : {rda_calories} kcal  (7-day average must be within 10-15% of this)
- Daily protein   : {rda_protein} g      (7-day average must be within 10-15% of this)
- Daily calcium   : {rda_calcium} mg     (7-day average must be within 10-15% of this)
- Daily iron      : {rda_iron} mg        (7-day average must be within 10-15% of this)

Each day's three meals combined (breakfast + lunch + dinner) MUST sum to approximately
these daily targets. Do NOT generate meals totalling less than {int(rda_calories * 0.85)} kcal/day.
Use energy-dense Karnataka foods (Ragi Mudde, Bisibelebath, Groundnut Chikki, Ghee Rice,
Horsegram Saaru, etc.) to meet calorie and protein goals within the ₹150/day budget.
...
def generate_groq_plan(
    ...
    try:
        ...
        # ── Nutritional adequacy warning ──────────────────────────────────────
        rda           = ICMR_RDA.get(age_group, ICMR_RDA["9-12"])
        rda_cal_floor = rda["calories"] * 0.70   # 70% of ICMR target
        if avg_daily_cal < rda_cal_floor:
            print(
                f"[groq_engine] ⚠️  NUTRITION WARNING: avg_daily_cal={avg_daily_cal} kcal "
                f"for {student_name} (age {age_group}) is below 70% of ICMR RDA "
                f"({rda['calories']} kcal) — threshold={rda_cal_floor:.0f} kcal. "
                f"Plan was generated by Groq; consider regenerating."
            )
